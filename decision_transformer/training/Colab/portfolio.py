...
class portfolio():
  ...
  def add_spread(self, close_price, symbol):
    ...
    ask_price = close_price + self.spread_dict[symbol]*0.5
  ...
  def update_value(self, close_values, action_dict, volatility): # close_value is the exit value
    ...
    current_reward = np.log(current_value/self.prev_value)
    value_reward = np.log(self.value)

    reward = value_reward + 0.4*current_reward
